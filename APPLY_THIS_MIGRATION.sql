...
COMMENT ON TABLE public.organizations IS 'Business organizations using the platform';
COMMENT ON COLUMN public.organizations.handle IS 'Unique URL-friendly identifier for public booking pages';
COMMENT ON COLUMN public.organizations.timezone IS 'IANA timezone identifier for scheduling';
...
COMMENT ON TABLE public.memberships IS 'User organization memberships - UNIQUE constraint enforces single-business rule';
COMMENT ON COLUMN public.memberships.user_id IS 'UNIQUE constraint ensures one user can only belong to one organization';
...
COMMENT ON TABLE public.invites IS 'Pending staff invitations';
